#needed a new script to grab noisy LCs from random tess data as more negatives 
...
N_RANDOM_TOTAL = 150  #total number of noisy negatives i want
LC_LENGTH = 1000
OUTPUT_BASE = "processed_lcs"
...
for split in splits:
    os.makedirs(os.path.join(OUTPUT_BASE, split, "negative/noisy"), exist_ok=True)
...
def process_tic(tic, sector):
    ...
    try:
        ...
        for i in filtered_indices:
            ...
            #save to random split of train/val/test
            split = get_split()
            outdir = os.path.join(OUTPUT_BASE, split, "negative/noisy")
